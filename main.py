...
def criar_card(titulo, subtitulo, caminho_imagem, emoji_fallback, chave_modulo):
    ...
    with st.container(border=True):
        # ── Imagem: inline style garante largura total, sem interferência do CSS externo ──
        if img_src:
            st.markdown(f"""
            <div style="width:100%; height:115px; border-radius:4px; overflow:hidden;
                ...
                <img src="{img_src}" style="width:100%; height:100%;
                           object-fit:cover; display:block; opacity:0.95;">
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
        ...
        # ── Botão nativo (o CSS acima garante branco/preto) ──
        if st.button(titulo, key=f"btn_{chave_modulo}", use_container_width=True):
            st.session_state['modulo_ativo'] = chave_modulo
            st.rerun()

        # ── Horário centralizado via inline style ──
        st.markdown(f"""
        <div style="font-size:11px; color:#c9d1d9; line-height:1.5; font-weight:500;
