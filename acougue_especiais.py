...
def carregar_pedidos():
    try:
        ...
        if "Spreadsheet must be specified" in str(e):
            st.error("🚨 **Erro Crítico:** URL da Planilha não especificada.")
            st.stop()
        else:
            raise e
    except Exception as e:
...
def iniciar_tela():
    ...
    @media print {
        ...
        #print-section h2 {
            font-size: 13px !important; margin: 0 0 6px 0 !important; padding-bottom: 3px !important;
            border-bottom: 2px solid #000 !important; color: #000 !important; display: block !important;
            text-align: center !important;
        }
        #print-section h3 {
            font-size: 10px !important; font-weight: 700 !important; border-bottom: none !important;
            margin-top: 12px !important; margin-bottom: 3px !important; color: #000 !important;
        }
        .print-container { width: 100% !important; display: block !important; }

        table.print-table {
            width: 100% !important; border-collapse: collapse !important; color: #000000 !important;
            font-family: 'IBM Plex Sans', sans-serif !important; line-height: 1.2 !important;
            display: table !important; table-layout: fixed !important; margin-bottom: 4px !important;
        }
        table.print-table th, table.print-table td {
            border: 1px solid #444 !important; padding: 2px 3px !important; color: #000000 !important;
            background-color: #ffffff !important; overflow: hidden !important; text-overflow: ellipsis !important;
        }
        table.print-table td { white-space: nowrap !important; }
        table.print-table th {
            background-color: #d5d5d5 !important; font-weight: bold !important; text-align: center !important;
            white-space: normal !important; word-break: break-word !important; vertical-align: middle !important;
        ...
        table.print-loja th:nth-child(6), table.print-loja td:nth-child(6) {
            width: 16% !important; text-align: center !important; font-weight: bold !important; background-color: #eeeeee !important;
            -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important;
        }
        ...
        table.print-forn th:nth-child(n+3):nth-child(-n+10), table.print-forn td:nth-child(n+3):nth-child(-n+10) { width: 6.5% !important; text-align: center !important; }
        table.print-forn th:nth-child(11), table.print-forn td:nth-child(11) {
            width: 8% !important; text-align: center !important; font-weight: bold !important; background-color: #eeeeee !important;
            -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important;
        }
        ...
        table.print-sep th:nth-child(12), table.print-sep td:nth-child(12) {
            width: 7% !important; text-align: center !important; font-weight: bold !important; background-color: #eeeeee !important;
            -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important;
        }
    }
    ...
    # SIDEBAR INTERNA
    with st.sidebar:
        st.markdown("### Módulo: Açougue Especial")
        if acesso_total:
    ...
    if perfil_navegacao == "Separação e Fechamento":
        ...
        with st.container(border=True):
            ...
            df_editado = st.data_editor(
                df_base[cols_order], hide_index=True, use_container_width=True, height=600,
                column_config=col_cfg, key=f"sep_editor_{st.session_state['reset_counter_acougue_especial']}"
            )

            # Ocultando Data/Hora local de impressão da preview, ajustado p/ ser mais limpo
            ...
            st.markdown(f"""<div id="print-section"><h2 style="color: black; margin-bottom: 10px; text-align: center; border-bottom: 2px solid black; padding-bottom: 5px;">
                Visão por Fornecedor (Resumo) — Açougue Especial &nbsp;&nbsp;&nbsp; <span style="font-size: 11px;">{agora}</span>
            </h2><div class="print-container">{html_table}</div></div>""", unsafe_allow_html=True)
            ...
            with col_salvar:
                if st.button("💾 Salvar Alterações", type="primary", use_container_width=True):
                    salvar_pedidos(df_editado.drop(columns=["TOTAL GERAL"]))
                    st.success("✅ Pedidos salvos na nuvem com sucesso!")
                    st.rerun()

            with col_csv:
                csv = df_editado.copy().rename(columns=MAPA_LOJAS).to_csv(index=False).encode("utf-8")
                st.download_button("⬇️ CSV", data=csv, file_name="separacao_acougue_especial.csv", mime="text/csv", use_container_width=True)

            with col_excel:
                excel_data = gerar_excel_estilizado(df_editado.copy().rename(columns=MAPA_LOJAS), "Separação Acougue")
                st.download_button("⬇️ Excel", data=excel_data, file_name="separacao_acougue_especial.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    ...
    # ROTA 2 — VISÃO DAS LOJAS
        ...
        df_loja_view = pd.merge(df_cat_loja[["Fornecedor", "Código", "Descrição"]], df_all[["Fornecedor", "Código", loja_selecionada]], on=["Fornecedor", "Código"], how="left")
        ...
        try:
            conn_pg = st.connection("banco_erp", type="sql")
            query_erp = f"""
                SELECT cade_codempresa, cade_codigo, cadp_descricao, estoque
        ...
        if not df_media_all.empty and "loja" in df_media_all.columns:
            ...
            df_media_loja = df_media_loja.rename(columns={"codigo": "Código", "qtde_media_semanal": "Média 90d"}).drop_duplicates(subset=["Código"])
        ...
        with st.container(border=True):
            st.info("💡 **Dica:** O **Estoque** foi preenchido automaticamente. Você pode preencher apenas a **Qtde** do pedido.")
            ...
            df_editado = st.data_editor(
                ...
                height=520, key=f"loja_acougue_especial_{st.session_state['reset_counter_acougue_especial']}"
            )

        html_table_loja = df_editado.copy().rename(columns={"Estoque": "Est.", "Média 90d": "Méd.90d", "Qtde": "Ped."}).to_html(index=False, classes=["print-table", "print-loja"])
        st.markdown(f"""<div id="print-section"><h2 style="color: black; margin-bottom: 10px; text-align: center; border-bottom: 2px solid black; padding-bottom: 5px;">
            Resumo do Pedido — {loja_selecionada}
        </h2><div class="print-container">{html_table_loja}</div></div>""", unsafe_allow_html=True)
        ...
        with col_aviso:
            st.write("<br>", unsafe_allow_html=True)
            if st.button("🚫 Sem Pedido", use_container_width=True):
                df_main = carregar_pedidos()
                df_main[loja_selecionada] = 0
                salvar_pedidos(df_main)
                msg_aviso = f"🚨 *AVISO - Açougue Especial*\nA {loja_selecionada} informou que *NÃO* fará pedido nesta semana."
                enviado = notificar_telegram(msg_aviso)
                if enviado: st.success("✅ Pedido zerado e Supervisor avisado!")
                else: st.warning("⚠️ Pedido zerado (Falha no Telegram).")
    ...
    # ROTA 3 — VISÃO POR FORNECEDOR (RESUMO)
        ...
        if df_all.empty or df_cat.empty:
            st.warning("Não há dados de pedidos.")
            st.stop()
        ...
        for i in range(0, len(nomes_forn), 1):
            ...
            for j, fornecedor in enumerate(nomes_forn[i:i+1]):
                ...
                df_forn = df_forn[["Código", "Descrição"] + LOJAS].copy()
                df_forn["TOTAL"] = df_forn[LOJAS].sum(axis=1)
                
                col_cfg_forn = {"Código": st.column_config.NumberColumn("Cód.", width=80, disabled=True), "TOTAL": st.column_config.NumberColumn("TOTAL", disabled=True)}
                for c in LOJAS: col_cfg_forn[c] = st.column_config.NumberColumn(c, min_value=0)

                with cols[j]:
                    with st.container(border=True):
                        st.markdown(f"#### 🍗 {fornecedor}")
                        df_forn_edit = st.data_editor(df_forn, hide_index=True, use_container_width=True, column_config=col_cfg_forn, key=f"forn_{fornecedor}_{st.session_state['reset_counter_acougue_especial']}")
                        total_geral = int(df_forn_edit["TOTAL"].sum()) if "TOTAL" in df_forn_edit.columns else 0
                        st.markdown(f"<div style='text-align:right; font-weight:700; color:var(--red-bright);'>Total Geral: {total_geral} unid.</div>", unsafe_allow_html=True)

                        html_table = df_forn_edit.to_html(index=False, classes=["print-table", "print-forn"])
                        html_print_content += f"<h3>🍗 {fornecedor}</h3>\n{html_table}\n<div style='text-align:right; font-weight:bold; font-size:11px;'>Total: {total_geral} unid.</div>\n"

        st.markdown(f"""<div id="print-section"><h2 style="color: black; text-align: center; border-bottom: 2px solid black;">
            Visão por Fornecedor &nbsp;&nbsp;&nbsp; <span style="font-size: 11px;">{agora}</span>
        </h2><div class="print-container">{html_print_content}</div></div>""", unsafe_allow_html=True)
        ...
        with col_csv:
            ...
            st.download_button("⬇️ CSV", data=csv_str, file_name="visao_fornecedor.csv", mime="text/csv", use_container_width=True)
        with col_excel:
            excel_data = gerar_excel_estilizado(df_export, "Visão Fornecedores")
            st.download_button("⬇️ Excel", data=excel_data, file_name="visao_fornecedor.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        with col_print:
    ...
    # ROTA 4 — CATÁLOGO DE PRODUTOS
        ...
        with st.container(border=True):
            opcoes_forn = ["PIONEIRO", "FRANGO PARANÁ", "BIG FRANGO - TEMPERADOS", "BIG FRANGO - MIX BANDEJAS", "BIG FRANGO - MIX BALCÃO", "BIG FRANGO - MIX CONGELADO"]
            col_cfg_cat = {
                ...
                "Descrição Oficial":  st.column_config.TextColumn("Nome Oficial (ERP)", width=280),
                "Nome Personalizado": st.column_config.TextColumn("Nome Personalizado", width=230),
            }
            for loja in LOJAS: col_cfg_cat[loja] = st.column_config.CheckboxColumn(loja, default=False)

            edited_cat = st.data_editor(
                df_editor_input, use_container_width=True, hide_index=True, height=600, num_rows="dynamic",
                column_config=col_cfg_cat, key=f"editor_catalogo_acougue_{st.session_state['reset_counter_acougue_especial']}"
            )
        ...
        with col_sync:
            if st.button("📥 Puxar Nomes do ERP", use_container_width=True):
                try:
                    ...
                    df_nomes = conn_pg.query(f"SELECT cadp_codigo, cadp_descricao FROM cadprod WHERE cadp_codigo IN {cods_str}", ttl=0)
                    ...
                    salvar_catalogo(edited_cat)
                    st.success("✅ Nomes Oficiais sincronizados!")
                    st.rerun()
                except Exception as e:
